...
//printf("outside multiprocessor if block\n");
	...
	if ((argc > 1) && (strcmp(argv[1], "") != 0)) {
		...
		//			Validate_Process_Data(Create_Process_Data);
					...
					{
						printf("%d Exiting!!! As error has occurred", status);
						return;
					}
...
printf("before START_NEW_CONTEXT_AND_SUSPEND\n");
	mmio.Field2 = START_NEW_CONTEXT_AND_SUSPEND;
printf("Outside START_NEW_CONTEXT_AND_SUSPEND\n");
//printf("calling MEM_WRITE\n");
//printf("%d %x %d", &mmio, &mmio, Z502Context);
	MEM_WRITE(Z502Context, &mmio);     // Start up the context
printf("after MEM_WRITE\n");
